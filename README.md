...
- **7.16.5:** on the portfolio, the ball on the speed graph set off the moment the page did — out ahead of a curve that draws itself in over the best part of two seconds, so it travelled over a line that was not there and the path caught up behind it. It now waits for the line. The wait is asked of the animation rather than counted off against the duration in the stylesheet, because the two are not the same clock: a CSS animation starts when its element first renders, and on the live page the curve was still unstarted at forty-eight milliseconds while the ball had already left the first keyframe.
- **7.16.4:** the EN and BN letters sat a pixel high in their pill. `align-items: center` centres the line box, and the line box is not where the letters are: Noto Serif Bengali keeps two thirds of its em above the baseline because Bengali hangs matras up there, and a capital has nothing below the baseline to fill the third that is left. Measured on the live page — ink centre 4.5px against a box centre of 5.5px — and a pixel moved from the bottom padding to the top, so the pill is the height it always was and only the letters shifted. The pill on an article is unaffected: English and বাংলা both have ink below the baseline, and measured dead centre already.
...
├── src/
│   ├── css/               Stylesheet sources, one file per section (built in name order)
│   └── js/                JavaScript sources, one file per feature (share one closure)
├── scripts/build.js       Joins and minifies src/ into assets/*.min.*
├── assets/
│   ├── style.min.css      Built stylesheet (committed; do not edit by hand)
│   ├── app.min.js         Built JavaScript (committed; do not edit by hand)
...
The site loads `assets/style.min.css` and `assets/app.min.js`, which are generated from `src/css/` and `src/js/`. After editing anything under `src/`:
...
Then bump the version in `functions.php` (`RS_VERSION`), `style.css` (`Version:`), `package.json` and the README badge, and commit the built files together with the sources: the theme updates itself straight from this repository, so what is committed is what the live site serves. `npm run check` (also run by CI) fails when the committed build is out of date.
